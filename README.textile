...
p. For the setup we'll assume your table is named `jobs` although this is a configurable option.  If you do plan on using a different table name (eg. djjobs_jobs), make sure your SQL for CREATE TABLE reflects that.
...
bc. DJJob::configure("mysql:host=127.0.0.1;dbname=djjob_test;port=3306", "root", "topsecret", "table_name");
...
p. @table_name@ is an optional parameter in the event that you have created your table using a different name.
